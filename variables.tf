...
variable "alb_domain_name" {
  description = "The DNS name of the ALB. Use if your Application Load Balancer is public"
  type        = string
  default     = null
  nullable    = true
}

variable "alb_vpc_origin" {
  type = object({
    id                         = string
    dns_name                   = string                    // The Load Balancers autogenerated DNS name
    custom_headers             = optional(map(string), {}) // Optional Headers that is passed to the origin
    keep_alive_timeout_seconds = optional(number)          // How long CloudFront persists its connection to the origin. Default 5s, max is 60s
    read_timeout_seconds       = optional(number)          // How long CloudFront waits for a response from the origin. Default 30s, max is 60s
    owner_account_id           = optional(string)          // Use only if the VPC origin lives in another AWS Account
  })
  description = "Use VPC origin if your Load Balancer lives inside a Private Subnet"
  default     = null
  nullable    = true
}
